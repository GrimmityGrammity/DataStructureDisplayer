...
public class TreeNode implements TreeNodeADT{
    ...
    final public static Color SELECTED_COLOR = Color.blue; //color to show selected node in
   ...
   public int depth(){
	...
	public int getAsciiValue(){
		if(this instanceof TrieNode && getData() != null){
			return (int) getData().key().charAt(0);
		} else {
			return 63;
		}
	}
    ...
    public void addChildNode(TreeNode aNode){
    	...
    	// For the trie
    	if(aNode instanceof TrieNode){
    		if(children.size() > 0){
    			// Find the new node's alphabetical place
    			for(int i = 0; i < children.size(); i ++){
    				// New node comes before
    				if(children.get(i) != null && aNode.getAsciiValue() < children.get(i).getAsciiValue()){
    					children.add(i, aNode);
    					return;
    				}
    				// New node comes after
    				else if(children.get(i) != null && aNode.getAsciiValue() > children.get(i).getAsciiValue()){
    					continue;
    				}
    				// New node is equal
    				else if(children.get(i) != null && aNode.getAsciiValue() == children.get(i).getAsciiValue()){
    				...
    				// New node has found null for some reason, get will return npe anyway, but w/e
    				else if(children.get(i) == null){
    					children.add(aNode);
    					return;
    				}
    				// We have reached the end without placing anything
    		...
    		// There was no children to begin with
			else {
				children.add(aNode);
			}
    	}
   ...
   public void removeChildNode(TreeNode aChildNode){
	...
	public void draw(Graphics2D aPen) {
		 ...
		 if(this instanceof TrieNode){
	    	 TrieNode temp = (TrieNode) this;
	    	 if(temp.getWord() != null && temp.getWord().length() > 0){
	    		
	    		 aPen.setColor(Color.BLACK);
	    		// Print out the word beside the node
	    		 aPen.drawString(temp.getWord(), location.x + (TreeNode.RADIUS * 2) - 10, location.y + 15);
	    		 aPen.setColor(getDrawingColor());
		 ...
		 else if(this instanceof RBTreeNode) {
			 RBTreeNode temp = (RBTreeNode) this;
     ...
     public void drawNodeLabel(Graphics aPen) {
	     ...
	     if(TreeEditor.displayDataValues && this instanceof TrieNode){
	    	 TrieNode temp = (TrieNode) this;
	    	 if(temp.getWord() != null && temp.getWord().length() > 0){
	    		 aPen.drawString(temp.getWord(), location.x + (RADIUS * 2), location.y - RADIUS);
	    	 }
	     }
